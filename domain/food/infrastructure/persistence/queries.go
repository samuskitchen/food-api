...
const(
	...
	selectFoodByUserId = "SELECT id, user_id, title, description, food_image, created_at, updated_at FROM food WHERE user_id = $1;"

	// insertFood is a query that inserts a new row in the user table using the values
	// given in order for user_id, title, description, food_image, created_at, updated_at.
	insertFood = "INSERT INTO food (user_id, title, description, food_image, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id;"
